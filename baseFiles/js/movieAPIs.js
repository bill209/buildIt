app.factory('xbmcFactory', function($q, $http) {
	return {
		getMoviesFromXBMCApi: function(){
			var jsonStr = 'jsonrpc?request={"jsonrpc": "2.0", "method": "VideoLibrary.GetMovies", "params": {"limits": { "start" : 0, "end": 2 },"properties": ["dateadded", "title", "originaltitle", "playcount", "year", "genre", "studio", "country", "tagline", "plot", "runtime", "file", "plotoutline", "lastplayed", "trailer", "rating", "resume", "art", "streamdetails", "mpaa", "director"], "sort": { "order": "ascending", "method": "label", "ignorearticle": true } }, "id": "libMovies"}&callback=JSON_CALLBACK';
			var deferred = $q.defer();
			$http
				.jsonp(xbmcIP + jsonStr)
				.success(function(e){
					console.log('e',e);
				})
...
console.log('d',d);
		...
		getMoviesFromLocalJson: function(movies){
			var deferred = $q.defer();
			$http
				.get(movies)
				.then(function(d){
